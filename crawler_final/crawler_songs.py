# -*- coding: utf-8 -*-
import requests
import json
import re
import os
import io
import sys
import sqlite3
from bs4 import BeautifulSoup
import requests
import time
import random
import sqlite3
from fake_useragent import UserAgent

import comments

script_dir = os.path.dirname(os.path.abspath(__file__))
os.chdir(script_dir)
ua = UserAgent()

headers = {
    "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
    "accept-language": "zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6",
    "cache-control": "max-age=0",
    "priority": "u=0, i",
    "sec-ch-ua": "\"Not/A)Brand\";v=\"8\", \"Chromium\";v=\"126\", \"Microsoft Edge\";v=\"126\"",
    "sec-ch-ua-mobile": "?0",
    "sec-ch-ua-platform": "\"Windows\"",
    "sec-fetch-dest": "document",
    "sec-fetch-mode": "navigate",
    "sec-fetch-site": "same-origin",
    "sec-fetch-user": "?1",
    "upgrade-insecure-requests": "1",
    "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36 Edg/126.0.0.0"
}

csrf_token = '634edee8766709b69e9d167c72d759e0'

cookies1 = {
    "NMTID": "00O50xBUphWIaHLQkBxr8aWOtpYEH8AAAGQnhIQAg",
    "JSESSIONID-WYYY": "iFTBD%2FToVWFF7X%2FSTqDj4YTQIUuAwryk%2BVrc%5Cf5Ee9X7sMJosBgzOPy%5CSk5r6Q5CPCP%2BRNvoNGMwmzaFsPAkkNAXcEXe%2BmAoNgAyddNz7D8KgJAwxnYb75OXyAD%5CpncDGkxNIJM%2BsBICZ0Xzv3jFphigsT2YjW5E%5Ck%5CTh8M%2Bi4moPt9u%3A1720640702123",
    "_iuqxldmzr_": "32",
    "_ntes_nnid": "33fb20c76918f5f102af858e5acd5d19,1720638902144",
    "_ntes_nuid": "33fb20c76918f5f102af858e5acd5d19",
    "WEVNSM": "1.0.0",
    "WNMCID": "jeyxei.1720638902927.01.0",
    "WM_NI": "CvKoKH0f87v6ZbDEB9aN7vAHN14WOixP3LRns352v%2Fd3Di9AAXoLXrxsPHvyQSnwtUiuG7m5VPS1biWL5R6qHQF2viuudY7N8KRxXmYO8YdO5qBSOMyOsNlirgDXP2K2YVU%3D",
    "WM_NIKE": "",
    "WM_TID": "JWKkse%2BpoX5FRQFQRFLGVIf5ElgWNMlQ",
    "sDeviceId": "YD-VApG0A9tBB9AQhEQBFKSFDkHMVljieZT",
    "ntes_utid": "tid._.yY13lEt%252BzcNBFgBQBAfSFZP4Rl1Dzv3w._.0"
}

cookies2 = {
    "NMTID": "00OnBJmg4aEEfLa30ZkhhZJOPZ05WkAAAGQnalYRw",
    "JSESSIONID-WYYY": "eYeIWjgMujfIsW2d5dgxxiOPgtPVp2X2m9zhgOvOW%2FtQtxqXynXMGOolZ2obBsPxv6%5Cq7jwMFJsQCGpN3%2B%2FbJZ41H4KhjpT7z%5CEaWU5GYrxtPW9oqVm58pXBhthDvDOaxvz1G4VWHdS%2Ftv%2FFAjXAyWB6QQikZ3RcYrzyCFkMpQ6SVinh%3A1720633839689",
    "_iuqxldmzr_": "32",
    "_ntes_nnid": "95f5b8c34bdb303dd9ca4a7ae1f35750,1720632039702",
    "_ntes_nuid": "95f5b8c34bdb303dd9ca4a7ae1f35750",
    "WEVNSM": "1.0.0",
    "WNMCID": "thkabg.1720632041706.01.0",
    "WM_NI": "m8%2F95Vq3Te0mzIymDV3P97MGCazPGBczt156G4XrFWkf4ZLhnCL4FjsvjmmLs1tLwot%2B7VQURThDsE5USKw5WSClBypgak95aL%2BUZ6BS1A0dLGItb6fLLtbAOdB4yIUnYmo%3D",
    "WM_NIKE": "",
    "WM_TID": "7y0%2BmsjaPxlEEVRUAQKCUdK3jYcoYM8z",
    "sDeviceId": "YD-MR7QQT25ZQhBUhVEUROGQZO3nJI8gx2K",
    "ntes_utid": "tid._.mY0%252FvFtUD5ZEA1BFVQLDAYan3NYolg2G._.0"
}

cookies3 = {
    "NMTID": "00OkHxRDwGmGRHaFkCunyllUMUt_bMAAAGQnhHuOw",
    "JSESSIONID-WYYY": "Hagh8IOO%2FG9m3vn0nf4V%2F2SB%5CqlhYtbtdi6wsV6M1Ax9UvVUWxN%2FUh79ZQQti1u6JoFMQoOONZmY7q9ryMSWRUCOSwGF8FgU7jyK6Td%2FJvxX%2By3wZokvC5coEoYF5bodHKiRUhqFx%2BOTv88bdGn1iPv1l%2FY07iiogThd678x49j4zq4j%3A1720640693767",
    "_iuqxldmzr_": "32",
    "_ntes_nnid": "c7283d23c34c50ec5a735785ee82f1b2,1720638893791",
    "_ntes_nuid": "c7283d23c34c50ec5a735785ee82f1b2",
    "WEVNSM": "1.0.0",
    "WNMCID": "xvrgka.1720638894459.01.0",
    "WM_NI": "P2aaWU3y2csaconlX3suPkOXmqMuqpp8WOU4Dv%2BPMhZjDHIF0tyzu%2F%2Ba9yqdExm6wu8x41o3OrwF2CSsgp7MyQkDVJtww%2FDSZ1Kq6hVQkgxtuYIDXRStsuf3%2B2FnFeGMdkc%3D",
    "WM_NIKE": "",
    "WM_TID": "hMjRk9uwvZBEURQRFRfSFYOtV0a66HLb",
    "sDeviceId": "YD-vP4l6t64O0NAR0AAFELCQJPoFwxEAvRw",
    "ntes_utid": "tid._.2Jv3BUo%252FaopAA0UFUBbHBYfoF1xEcuRJ._.0"
}

cookies4 = {
    "NMTID": "00OqPKOtTnKRKTcmEZxrNbm3dNNdCYAAAGQnl0zmg",
    "JSESSIONID-WYYY": "Dmj%2FOf7zN9KVXF0fdxZf%5CRcnIwiUo%2Bh4YioUYjaRC5E6T9FN6Ud%2BPrP5ilmXiiu%2F%2BG7%2FGmezJPOzMSSFuRRUz5k5NcnFZRsxcFNWwFm2JjIFjpiveQcRjOsIp7%2B%5CspZMcnkj4BMsban2AxN4q4Cy2RihxpHTPsw8bvB1YtvHZwhxEKMt%3A1720645627080",
    "_iuqxldmzr_": "32",
    "_ntes_nnid": "aefe0fd4ae5519d99226ad9b6b1e2a74,1720643827094",
    "_ntes_nuid": "aefe0fd4ae5519d99226ad9b6b1e2a74",
    "WEVNSM": "1.0.0",
    "WNMCID": "jyzemu.1720643829526.01.0",
    "WM_NI": "hzhjBZ%2Fhemky8a%2FMNsUmiivrjRfd5XSLZ3o4lCxYVRt4tFvJ1%2FXMfanGLOP45p9Faa4%2FMpIPtrEaMEZ2ev1P3ClR31fYhyb2SUdXzTFEwG8q4zwmyz1cpxNIVJUFvnxJbVA%3D",
    "WM_NIKE": "",
    "WM_TID": "xTH1IoMtW2BBVVEAQQeGVdOoI%2FYwDpwa",
    "ntes_utid": "tid._.1bXwBg0qx%252BVEBxEVFRLTAZP9J7JhTzNE._.0",
    "sDeviceId": "YD-w9oafKFMc%2FpBRgAAQVbGENKtJrZhXrl8"
}

cookies_list = [cookies1, cookies2, cookies3, cookies4]

conn = sqlite3.connect('database.db')
cursor = conn.cursor()

cursor.execute('''CREATE TABLE IF NOT EXISTS music
               (id INTEGER PRIMARY KEY, url TEXT, music_name TEXT, sub_music_name TEXT, 
               album_name TEXT, album_id TEXT,
               artist_name TEXT, artist_id TEXT, sub_artist_name TEXT, sub_artist_id TEXT, artist_cnt INTEGER,
               lyrics TEXT, lyrics_pure TEXT, translated_lyrics_pure TEXT,
               comment TEXT, img_url TEXT)''')

conn.commit()

song_saves = []
with open('song_saves.txt', 'r', encoding='utf-8') as file:
    for line in file:
        song_saves.append(line.strip())

loading = int(1)
artist_saves = set()
img_folder = 'img'
if not os.path.exists(img_folder):
    os.makedirs(img_folder)

for key in song_saves[:]:

    if loading % 50 == 0:
        print("Small rest...")
        time.sleep(10)
    if loading % 500 == 0:
        print("Big rest...")
        time.sleep(30)

    print("ID: " + key + " , Process: " + str(loading) + "/" + "2801, " + str(format(float(loading) * 100/2801, '.2f')) +"%")
    url = "https://music.163.com/song?id=" + key   # url get
    url_save = "https://music.163.com/#/song?id=" + key

    res = requests.get(url, headers=headers, cookies=random.choice(cookies_list))
    soup_now = BeautifulSoup(res.text, features="html.parser")

    music_name_get = soup_now.find("em", class_="f-ff2")   # music name get
    music_name = music_name_get.text

    sub_music_name_get = soup_now.find("div", class_="subtit f-fs1 f-ff2")   # sub-music name get
    if sub_music_name_get:
        sub_music_name = sub_music_name_get.text
    else:
        sub_music_name = ""
    
    time.sleep(random.uniform(0,0.25))

    album_name_get = soup_now.find("a", class_="s-fc7", href=lambda value: value and value.startswith("/album?"))   # album name get
    if album_name_get:
        album_name = album_name_get.text
        album_id = album_name_get['href'][10:]
    else:
        album_name = ""
        album_id = ""

    # album_id_get = soup_now.find("a", class_="s-fc7", href=lambda value: value and value.startswith("/album?"))   # album id get
    
    artist_name_get = soup_now.find_all("a", class_="s-fc7", href=lambda value: value and value.startswith("/artist?"))   # author name & id get
    artist_name = ""
    artist_id = ""
    sub_artist_name = ""
    sub_artist_id = ""
    artist_cnt = len(artist_name_get)
    for index, author_tmp in enumerate(artist_name_get):
        if index == 0:
            artist_name = author_tmp.text
            artist_id = author_tmp['href'][11:]
            artist_saves.add(author_tmp['href'][11:])
        elif index == 1:
            sub_artist_name = author_tmp.text
            sub_artist_id = author_tmp['href'][11:]
            artist_saves.add(author_tmp['href'][11:])
        else:
            break

    # author_name_str = json.dumps(author_name, ensure_ascii=False)   # convert to json for saving
    # author_id_str = json.dumps(author_id, ensure_ascii=False)

    # author_id_get = soup_now.find("a", class_="s-fc7", href=lambda value: value and value.startswith("/artist?"))
    # author_id = 

    # time.sleep(random.uniform(0,0.5))

    img_get = soup_now.find("img", class_="j-img")   # image url get
    img_url = img_get['data-src']
    response = requests.get(img_url)
    if response.status_code == 200:
        img_path = os.path.join(img_folder, f"{key}.jpg")
        with open(img_path, 'wb') as file:
            file.write(response.content)



    lyrics_url = "https://music.163.com/api/song/lyric?id=" + key + "&lv=1&kv=1&tv=1"   # lyrics get
    lyrics_now = requests.get(lyrics_url, headers = headers, cookies=random.choice(cookies_list))
    lyrics_now.encoding = 'utf-8'
    lyrics_json = json.loads(lyrics_now.text)
    lyrics = {}
    regex = re.compile(r'\[.*\]')
    pattern = re.compile(r'(\[.*?\])([^\[]*)')
    if ('lrc' in lyrics_json):
        lyrics_pure_str = re.sub(regex, '', lyrics_json['lrc']['lyric']).strip()

        matches = pattern.findall(lyrics_json['lrc']['lyric'])
        for match in matches:
            lyrics[match[0]] = match[1].strip()
    else:
        lyrics_pure_str = ""
    if ('tlyric' in lyrics_json):
        translated_lyrics_pure_str = re.sub(regex, '', lyrics_json['tlyric']['lyric']).strip()

        matches = pattern.findall(lyrics_json['tlyric']['lyric'])
        for match in matches:
            if match[0] in lyrics:
                lyrics[match[0]] += " | " + match[1].strip()
    else:
        translated_lyrics_pure_str = ""
    lyrics_str = json.dumps(lyrics, ensure_ascii=False)   # convert to json for saving


    time.sleep(random.uniform(0,1))
    

    comment = comments.get_comment_api(key)   # comments get
    comment_str = json.dumps(comment, ensure_ascii=False)   # convert to json for saving

    cursor.execute('''INSERT INTO music (id, url, music_name, sub_music_name,
                    album_name, album_id, 
                    artist_name, artist_id, sub_artist_name, sub_artist_id, artist_cnt,
                    lyrics, lyrics_pure, translated_lyrics_pure, comment, img_url) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
                (int(key), url_save, music_name, sub_music_name, album_name, album_id, 
                artist_name, artist_id, sub_artist_name, sub_artist_id, artist_cnt,
                lyrics_str, lyrics_pure_str, translated_lyrics_pure_str, comment_str, img_url))
    conn.commit()

    loading += 1



# query = "SELECT * FROM music"
# try:
#     cursor.execute(query)
#     rows = cursor.fetchall()
#     for row in rows:
#         print(row)
# except sqlite3.Error as e:
#     print(f"Failed: {e}")
conn.close()
with open('artist_saves.txt', 'w', encoding='utf-8') as file:
    for artist in artist_saves:
        file.write(f"{artist}\n")

print("Completed.")